...
Public Class DeliveryOrder_O
    ...
    Private Sub DeliveryOrder_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        bm.Fields = New String() {"Flag", MainId, SubId, "DayDate", "ToId", "Notes", "Cashier", "DocNo", "Temp", "SalesInvoiceNo", "Value", "SaveId"}
        bm.control = New Control() {txtFlag, StoreId, InvoiceNo, DayDate, ToId, Notes, CashierId, DocNo, Temp, SalesInvoiceNo, Value, SaveId}
    ...
    Private Sub SaveId_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles SaveId.LostFocus
        Dim tbl As String = "Saves"
        bm.LostFocus(SaveId, SaveName, "select Name from " & tbl & " where Id=" & SaveId.Text.Trim())
    End Sub
    ...
    Private Sub txtID_KeyPress(ByVal sender As Object, ByVal e As Input.KeyEventArgs) Handles StoreId.KeyDown, InvoiceNo.KeyDown, ToId.KeyDown, CashierId.KeyDown, SaveId.KeyDown
    ...
    Private Sub DocNo_LostFocus(sender As Object, e As RoutedEventArgs) Handles DocNo.LostFocus
        Dim dt2 As DataTable = bm.ExecuteAdapter("GetSalesSerialDetails2", {"Flag", "StoreId", "DocNo", "SalesTypeIdState"}, {Flag, Val(StoreId.Text), Val(DocNo.Text), IIf(Md.MyProjectType = ProjectType.X, 0, 1)})
        G.Rows.Clear()
        ...
        Value.Text = bm.ExecuteScalar("select dbo.Bal0(AccNo,Id,getdate(),0,0,0) from Customers where ID=" & ToId.Text)

    End Sub

    Private Sub SalesInvoiceNo_LostFocus(sender As Object, e As RoutedEventArgs) Handles SalesInvoiceNo.LostFocus
        Return
        Dim dt2 As DataTable = bm.ExecuteAdapter("GetSalesSerialDetailsByInvoiceNo", {"Flag", "StoreId", "InvoiceNo"}, {Flag, Val(StoreId.Text), Val(SalesInvoiceNo.Text)})
        G.Rows.Clear()
        If dt2.Rows.Count = 0 Then Return
