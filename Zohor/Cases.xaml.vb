...
Public Class Cases
    ...
    Public Sub BasicForm_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        If IsSub Then
            ...
            btnAdmission.Visibility = Visibility.Hidden
            btnAttach.Visibility = Visibility.Hidden
            btnCloseTable.Visibility = Visibility.Hidden
            btnDischarge.Visibility = Visibility.Hidden
            btnDoctorInstruction.Visibility = Visibility.Hidden
            btnDownload.Visibility = Visibility.Hidden
            btnMortality.Visibility = Visibility.Hidden
            btnProgressNote.Visibility = Visibility.Hidden
            btnSetImage.Visibility = Visibility.Hidden
            btnSetNoImage.Visibility = Visibility.Hidden
        ...
        bm.Fields = New String() {SubId, "Name", "CityId", "Gender", "SSN", "Address", "DateOfBirth", "JobId", "Notes", "Manager", "BankAccount", "NationalId", "HomePhone", "Mobile", "Password", "EnName", "mm", "GeneralManager", "HasAttendance", "Accountant", "Board", "Cashier", "Waiter", "Deliveryman", "Occupation", "MenstrualH", "ObstetricH", "FamilyH", "OvarianFactor", "UtrineFactor", "TubalAndPeritoneal", "ObstHx", "HusbandName", "HusbandAge", "HusbandOccupation", "Risk", "GynEx", "MaleFactor", "PreviousART", "Email", "Address2", "OOR", "M2", "Sperm", "MaleCount", "Moitlity", "G3", "AbnormalForm", "Fertilization", "Empbryos", "ClassA", "Cryo", "Pregnancy", "CasePlan", "DateOfDeath", "DeathReason", "DiagnoseOnAdmission", "DiagnoseOnDischarge", "Summary", "NatureAndDuration", "HistoryofPresentIllness", "PastHistory", "FamilyHistory", "MedicalHistory", "HistoryOfOperation", "Medication", "Age", "CaseOutcomeId", "DateOnAdmission", "HegarDilator", "Recommendation", "Weight", "OperatedBefore", "AccNo", "Code"}
        bm.control = New Control() {txtID, ArName, CityId, Gender, SSN, Address, DateOfBirth, JobId, Notes, Manager, BankAccount, NationalId, HomePhone, Mobile, Password, EnName, mm, GeneralManager, HasAttendance, Accountant, Board, Cashier, Waiter, Deliveryman, Occupation, MenstrualH, ObstetricH, FamilyH, OvarianFactor, UtrineFactor, TubalAndPeritoneal, ObstHx, HusbandName, HusbandAge, HusbandOccupation, Risk, GynEx, MaleFactor, PreviousART, Email, Address2, OOR, M2, Sperm, MaleCount, Moitlity, G3, AbnormalForm, Fertilization, Empbryos, ClassA, Cryo, Pregnancy, CasePlan, DateOfDeath, DeathReason, DiagnoseOnAdmission, DiagnoseOnDischarge, Summary, NatureAndDuration, HistoryofPresentIllness, PastHistory, FamilyHistory, MedicalHistory, HistoryOfOperation, Medication, Age, CaseOutcomeId, DateOnAdmission, HegarDilator, Recommendation, Weight, OperatedBefore, AccNo, Code}
    ...
    Private Sub btnProgressNote_Click(sender As Object, e As RoutedEventArgs) Handles btnProgressNote.Click
        DontClear = True
        btnSave_Click(Nothing, Nothing)
        DontClear = False
        If Not AllowSave Then Return

        Dim frm As New MyWindow With {.Title = "Progress Note", .WindowState = WindowState.Maximized}
        Dim c As New ProgressNote
        c.MyCase = Val(txtID.Text)
        c.MyCaseName = EnName.Text
        frm.Content = c
        frm.ShowDialog()
    ...
    Private Sub btnAdmission_Click(sender As Object, e As RoutedEventArgs) Handles btnAdmission.Click
        DontClear = True
        btnSave_Click(Nothing, Nothing)
        DontClear = False
        If Not AllowSave Then Return
        ...
        frm.Rpt = "Admission.rpt"
        frm.paraname = {"Header", "@Id"}
        frm.paravalue = {"Admission Sheet", Val(txtID.Text)}
        frm.ShowDialog()
    End Sub

    Private Sub btnDischarge_Click(sender As Object, e As RoutedEventArgs) Handles btnDischarge.Click
        DontClear = True
        btnSave_Click(Nothing, Nothing)
        DontClear = False
        If Not AllowSave Then Return
        ...
        'Dim frm As New ReportViewer
        'frm.Rpt = "Discharge.rpt"
        'frm.paraname = {"Header", "@Id"}
        'frm.paravalue = {"Discharge Sheet", Val(txtID.Text)}
        'frm.ShowDialog()

        Dim rpt As New ReportViewer
        rpt.paraname = New String() {"@CaseId", "CaseName", "@Flag", "@MainId", "@DayDate", "@Id", "Header"}
        rpt.paravalue = New String() {Val(txtID.Text), ArName.Text, -2, 0, bm.ToStrDate(Now.Date), 0, "Patient History"}
        rpt.Rpt = "CaseAllDetails.rpt"
        rpt.Show()
    End Sub

    Private Sub btnMortality_Click(sender As Object, e As RoutedEventArgs) Handles btnMortality.Click
        DontClear = True
        btnSave_Click(Nothing, Nothing)
        DontClear = False
        If Not AllowSave Then Return
        ...
        Dim frm As New ReportViewer
        frm.Rpt = "Mortality.rpt"
        frm.paraname = {"Header", "@Id"}
        frm.paravalue = {"Mortality Sheet", Val(txtID.Text)}
        frm.ShowDialog()
    End Sub
    ...
    Private Sub AccNo_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles AccNo.LostFocus
        bm.AccNoLostFocus(AccNo, AccName, , MyLinkFlie, )
    End Sub
    ...
    Private Sub AccNo_KeyDown(ByVal sender As Object, ByVal e As Input.KeyEventArgs) Handles AccNo.KeyUp
        bm.AccNoShowHelp(AccNo, AccName, e, , MyLinkFlie, )
    End Sub
