...
Public Class Schedule
    ...
    Private Sub LoadAll()
        dt2 = bm.ExecuteAdapter("select Id,dbo.GetEmpArName(EmpId)EmpName,Notes,MyLine,dbo.ToStrDate(Daydate)Daydate,MyGetDate from Schedule order by MyGetDate desc")
        dt2.TableName = "tbl"
    ...
    Private Sub Calendar1_SelectedDatesChanged(sender As Object, e As SelectionChangedEventArgs) Handles Calendar1.SelectedDatesChanged
        dt = bm.ExecuteAdapter("select Id,dbo.GetEmpArName(EmpId)EmpName,Notes,MyLine,MyGetDate from Schedule where DayDate='" & bm.ToStrDate(Calendar1.SelectedDate) & "' order by MyGetDate desc")
        dt.TableName = "tbl"
    ...
    Private Sub btnDelete_Click(sender As Object, e As RoutedEventArgs) Handles btnDelete.Click
        If bm.ShowDeleteMSG("MsgDelete") Then
            bm.ExecuteNonQuery("delete Schedule where DayDate='" & bm.ToStrDate(Calendar1.SelectedDate) & "' and MyLine=" & MyLine)
    ...
    Private Sub btnSave_Click(sender As Object, e As RoutedEventArgs) Handles btnSave.Click
        If MyLine = 0 Then
            bm.ExecuteNonQuery("insert Schedule(DayDate,Id,EmpId,UserName,MyGetDate,Notes) select '" & bm.ToStrDate(Calendar1.SelectedDate) & "',(select isnull(MAX(Id),0)+1 from Schedule where DayDate='" & bm.ToStrDate(Calendar1.SelectedDate) & "')," & Md.UserName & "," & Md.UserName & ",GetDate(),'" & Notes.Text.Trim.Replace("'", "''") & "'")
        Else
            bm.ExecuteNonQuery("update Schedule set EmpId=" & Md.UserName & ",UserName=" & Md.UserName & ",MyGetDate=GetDate(),Notes='" & Notes.Text.Trim.Replace("'", "''") & "' where DayDate='" & bm.ToStrDate(Calendar1.SelectedDate) & "' and MyLine=" & MyLine)
        End If
        btnNew_Click(Nothing, Nothing)
    End Sub
