...
Public Class ReservationsRoomsPre
    ...
    Private Sub btnReservClick(ByVal sender As Object, ByVal e As RoutedEventArgs)
        ...
        Dim dt As DataTable = bm.ExecuteAdapter("select * from ReservationsRooms where RoomId='" & RoomId.SelectedValue.ToString & "' and DayDate='" & bm.ToStrDate(DateTime.Parse(DayDate.Content.ToString)) & "' and ReservId='" & lblReservId.Content.ToString & "'")
        ...
        If dt.Rows.Count > 0 Then
            ...
            OutDate.SelectedDate = dt.Rows(0)("OutDate")
            ...
            If Val(dt.Rows(0)("Posted").ToString) = 1 OrElse Val(dt.Rows(0)("IsClosed").ToString) = 1 Then
    ...
    Private Sub RoomId_SelectionChanged(ByVal sender As System.Object, ByVal e As System.Windows.Controls.SelectionChangedEventArgs) Handles RoomId.SelectionChanged
        LoadReservations()
    End Sub
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles btnSave.Click, btnSaveWithoutPrint.Click
        ...
        lblTime.Content = HH1.SelectedValue.ToString & ":" & MM1.SelectedValue.ToString & " " & Index1.Text

        Dim State As BasicMethods.SaveState = BasicMethods.SaveState.Update
        If SerialId.Text = "" Then State = BasicMethods.SaveState.Insert

        Dim MyCanceledDate As String = "1900-01-01"
        If Canceled.IsChecked Then
            MyCanceledDate = bm.MyGetDateTime()
        End If

        Dim str As String
        If IsNew Then
            str = "insert ReservationsRooms(RoomId,DayDate,ReservId,Time,CaseId,CaseName,OperationType,OperationType2,OperationType3,SurgeonId,SurgeonId2,SurgeonId3,AnesthetistId,NurseId,Value,Payed,Remaining,SerialId,UserName,MyGetDate,ReservationDate,Done,Canceled,Returned,RemainingDate,EmpIdReservation,EmpIdRemaining,EmpIdCanceled,CanceledDate,EmpIdReturned,ReturnedDate,HH1,MM1,Index1,HH2,MM2,Index2,OutDate,Notes) select '" & RoomId.SelectedValue.ToString & "','" & bm.ToStrDate(DateTime.Parse(DayDate.Content.ToString)) & "','" & lblReservId.Content.ToString & "','" & lblTime.Content.ToString & "','" & CaseId.Text & "','" & CaseName.Text & "','" & OperationType.SelectedValue.ToString & "','" & OperationType2.SelectedValue.ToString & "','" & OperationType3.SelectedValue.ToString & "','" & Val(SurgeonId.Text) & "','" & Val(SurgeonId2.Text) & "','" & Val(SurgeonId3.Text) & "','" & Val(AnesthetistId.Text) & "','" & Val(NurseId.Text) & "','" & Val(Value.Text) & "','" & Val(Payed.Text) & "','" & Val(Remaining.Text) & "','" & SerialId.Text & "','" & Md.UserName & "',GetDate(),GetDate(),'" & bm.IIf(Done.IsChecked, 1, 0) & "','" & bm.IIf(Canceled.IsChecked, 1, 0) & "','" & bm.IIf(Returned.IsChecked, 1, 0) & "','" & bm.ToStrDate(RemainingDate.SelectedDate) & "','" & EmpIdReservation.SelectedValue.ToString & "','" & EmpIdRemaining.SelectedValue.ToString & "','" & EmpIdCanceled.SelectedValue.ToString & "','" & MyCanceledDate & "','" & EmpIdReturned.SelectedValue.ToString & "','" & bm.ToStrDate(ReturnedDate.SelectedDate) & "'," & HH1.SelectedValue & "," & MM1.SelectedValue & "," & Index1.SelectedValue & "," & HH2.SelectedValue & "," & MM2.SelectedValue & "," & Index2.SelectedValue & ",'" & bm.ToStrDate(OutDate.SelectedDate) & "','" & Notes.Text.Replace("'", "''") & "'"
        Else
            str = "update ReservationsRooms set CaseId='" & CaseId.Text & "',CaseName='" & CaseName.Text & "',OperationType='" & OperationType.SelectedValue.ToString & "',OperationType2='" & OperationType2.SelectedValue.ToString & "',OperationType3='" & OperationType3.SelectedValue.ToString & "',SurgeonId='" & Val(SurgeonId.Text) & "',SurgeonId2='" & Val(SurgeonId2.Text) & "',SurgeonId3='" & Val(SurgeonId3.Text) & "',AnesthetistId='" & Val(AnesthetistId.Text) & "',NurseId='" & Val(NurseId.Text) & "',Value='" & Val(Value.Text) & "',Payed='" & Val(Payed.Text) & "',Remaining='" & Val(Remaining.Text) & "',Done='" & bm.IIf(Done.IsChecked, 1, 0) & "',Canceled='" & bm.IIf(Canceled.IsChecked, 1, 0) & "',Returned='" & bm.IIf(Returned.IsChecked, 1, 0) & "',RemainingDate='" & bm.ToStrDate(RemainingDate.SelectedDate) & "',SerialId='" & SerialId.Text & "',EmpIdReservation='" & EmpIdReservation.SelectedValue.ToString & "',EmpIdRemaining='" & EmpIdRemaining.SelectedValue.ToString & "',EmpIdCanceled='" & EmpIdCanceled.SelectedValue.ToString & "',CanceledDate='" & MyCanceledDate & "',EmpIdReturned='" & EmpIdReturned.SelectedValue.ToString & "',ReturnedDate='" & bm.ToStrDate(ReturnedDate.SelectedDate) & "',UserName='" & Md.UserName & "',MyGetDate=GetDate(),HH1=" & HH1.SelectedValue & ",MM1=" & MM1.SelectedValue & ",Index1=" & Index1.SelectedValue & ",HH2=" & HH2.SelectedValue & ",MM2=" & MM2.SelectedValue & ",Index2=" & Index2.SelectedValue & ",Outdate='" & bm.ToStrDate(OutDate.SelectedDate) & "',Notes='" & Notes.Text.Replace("'", "''") & "' where RoomId='" & RoomId.SelectedValue.ToString & "' and DayDate='" & bm.ToStrDate(DateTime.Parse(DayDate.Content.ToString)) & "' and ReservId='" & lblReservId.Content.ToString & "'"
        End If
    ...
    Function Delete() As Boolean
        If RoomId.SelectedIndex < 1 OrElse DayDate.Content.ToString = "" OrElse lblReservId.Content.ToString = "" Then
            Return False
        End If

        Return bm.ExecuteNonQuery("insert ReservationsRoomsCanceled select * from ReservationsRooms where RoomId='" & RoomId.SelectedValue.ToString & "' and DayDate='" & bm.ToStrDate(DateTime.Parse(DayDate.Content.ToString)) & "' and ReservId='" & lblReservId.Content.ToString & "'          delete ReservationsRooms where RoomId='" & RoomId.SelectedValue.ToString & "' and DayDate='" & bm.ToStrDate(DateTime.Parse(DayDate.Content.ToString)) & "' and ReservId='" & lblReservId.Content.ToString & "'")
    End Function
    ...
    Private Sub btnClick(ByVal sender As Object, ByVal e As RoutedEventArgs)
        ...
        bm.ShowHelp("", ToId, ToName, New KeyEventArgs(Keyboard.PrimaryDevice, Keyboard.PrimaryDevice.ActiveSource, 0, Key.F1), "select cast(ReservId as varchar(100)) Id,CaseName+' - '+c.Mobile+' - '+c.HomePhone Name from ReservationsRooms r left join Cases c on(r.CaseId=c.Id) where r.RoomId='" & RoomId.SelectedValue.ToString & "' and DayDate='" & bm.ToStrDate(Calendar1.SelectedDate) & "'")
    ...
    Sub LoadReservations()
        Try
            ...
            Dim DaysDt As DataTable = bm.ExecuteAdapter("select Saturday,Sunday,Monday,Tuesday,Wednesday,Thursday,Friday from Rooms where Id=" & RoomId.SelectedValue.ToString)
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                Dim dt2 As DataTable = bm.ExecuteAdapter("LoadReservationsRooms", New String() {"RoomId", "Daydate"}, New String() {Val(RoomId.SelectedValue.ToString), bm.ToStrDate(bm.FormatDate(dt.Rows(i)("Date")))})
                For i2 As Integer = 0 To dt2.Rows.Count - 1
                    ...
                    x2.Tag = bm.FormatDate(dt.Rows(i)("Date")).Replace("/", "_") & "," & dt2.Rows(i2)("Id").ToString & "," & dt2.Rows(i2)("Name").ToString
                    x2.Width = 50
                    x2.Height = 50
                    ...
                    x2.Content = dt2.Rows(i2)("Id").ToString & vbCrLf & dt2.Rows(i2)("Name").ToString
    ...
    Private Sub btnRoomIdClick(sender As Object, e As RoutedEventArgs)
        RoomId.SelectedValue = sender.Tag
        RoomId_SelectionChanged(Nothing, Nothing)
    End Sub

    Private Sub ShowRPT()
        Dim rpt As New ReportViewer
        rpt.paraname = New String() {"@RoomId", "@Date", "@Id", "Header"}
        rpt.paravalue = New String() {Val(RoomId.SelectedValue), DayDate.Content, Val(lblReservId.Content), CType(Parent, Page).Title}
        rpt.Rpt = "ReservationsRoomsONE.rpt"
        rpt.Print()
    End Sub
    ...
    Private Sub LoadResource()
        ...
        TabItemReservationsRooms.SetResourceReference(TabItem.HeaderProperty, "ReservationsRooms")
        TabItemCurrentReservation.SetResourceReference(TabItem.HeaderProperty, "Current Reservation")

        lblRoomId.SetResourceReference(ContentProperty, "Room")
    ...
    Private Sub btnPrintSchedule_Click(sender As Object, e As RoutedEventArgs) Handles btnPrintSchedule.Click
        ...
        rpt.Rpt = "OperationsRoomsAllPre.rpt"
        rpt.Show()
    End Sub

    Private Sub btnPrintSchedule2_Click(sender As Object, e As RoutedEventArgs) Handles btnPrintSchedule2.Click
        Dim rpt As New ReportViewer
        rpt.paraname = New String() {"Header"}
        rpt.paravalue = New String() {CType(Parent, Page).Title}
        rpt.Rpt = "OperationsRoomsAllPre2.rpt"
        rpt.Show()
    End Sub

    Private Sub btnPrintPatients_Click(sender As Object, e As RoutedEventArgs) Handles btnPrintPatients.Click
        Dim rpt As New ReportViewer
        rpt.paraname = New String() {"@RoomId", "@Daydate"}
        rpt.paravalue = New String() {Val(RoomId.SelectedValue), Calendar1.SelectedDate.Value}
        rpt.Rpt = "PrintReservationsRooms.rpt"
        rpt.Show()
    End Sub
    ...
    Private Sub GetRooms()
        bm.FillCombo("select Id,Name from Rooms where (InpatientDepartmentId=" & Val(InpatientDepartmentId.SelectedValue) & " or " & Val(InpatientDepartmentId.SelectedValue) & "=0) and (InpatientDepartmentsSubId='" & Val(InpatientDepartmentSubId.SelectedValue) & "' or '" & Val(InpatientDepartmentSubId.SelectedValue) & "'=0) union select 0 Id,'-' Name order by Name", RoomId)
        WR1.Children.Clear()
        For i As Integer = 1 To RoomId.Items.Count - 1
            Dim x As New Button
            x.Style = Application.Current.FindResource("GlossyCloseButton")
            x.Name = "lbl_" & RoomId.Items(i)("Id").ToString
            x.Tag = RoomId.Items(i)("Id")
            x.VerticalContentAlignment = VerticalAlignment.Center
            x.Width = 160
            x.Height = 50
            ...
            x.VerticalAlignment = VerticalAlignment.Top
            x.Cursor = Input.Cursors.Pen
            x.Content = RoomId.Items(i)("Name")
            x.ToolTip = x.Content
            x.Background = bf.btnNew.Background 'System.Windows.Media.Brushes.SkyBlue
            x.Foreground = System.Windows.Media.Brushes.Black
            AddHandler x.Click, AddressOf btnRoomIdClick
            WR1.Children.Add(x)
        Next
    End Sub
